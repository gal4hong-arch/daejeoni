...
| **데이터베이스** | **`DATABASE_URL`이 가리키는 DB**에 앱 데이터 전부 저장. 기본값·로컬 예시는 SQLite 파일(`sqlite:///./data/platform.db`). Supabase **Postgres**를 쓰려면 같은 스키마로 URL만 교체(아래 표 참고). |
| **인증** | Supabase Auth JWT(`Authorization: Bearer`) 또는 로컬 전용 `ALLOW_DEMO_USER_HEADER=true` + `X-Demo-User` — 인증은 Supabase와 연동될 수 있으나, **앱 테이블 저장소는 `DATABASE_URL`과 별개** |

### 2.1 멀티유저 데이터가 어디에 쌓이는지 (로컬 SQLite vs Supabase)

- **확인 방법**: `cap/backend/.env`(또는 실행 환경)의 **`DATABASE_URL`** 한 줄이 기준이다.  
  - `sqlite:///...` → **백엔드가 쓰는 서버 디스크의 SQLite 파일** (여러 `user_id` 행이 한 파일에 공존).  
  - `postgresql://...`(Supabase 풀러 URL 등) → **해당 Postgres**에 동일 테이블로 저장.

| 항목 | 저장 위치 | user 구분 |
|------|-----------|-----------|
| RAG **소스 목록**·문서 메타·청크 (`kb_documents`, `kb_chunks` 등) | 서버 DB (`DATABASE_URL`) | `user_id` 컬럼 |
| 사용자 **API 키** (유저 정보에서 저장) | 서버 DB `user_api_keys` | `user_id` + provider, `FERNET_KEY`로 암호화 |
| **모델·설정** (기본 모델, dual_api 등) | 서버 DB `user_model_preferences` | `user_id` PK |
| 대화·토픽·메시지·감사로그 등 | 동일 서버 DB | 각 테이블 `user_id` 또는 소유 관계 |
| RAG 패널 **문서 체크(선택)** 상태 | 브라우저 **localStorage** (`rag_doc_checks_v1:<user_id>`) | 기기·브라우저마다 다를 수 있음(서버의 “소스 목록”과 별개) |
| 검토 역할·페르소나·테마·사이드바 접힘 등 | 브라우저 **localStorage** | 서버 설정과 무관 |

**정리**: Supabase 프로젝트에 `SUPABASE_URL` / `SUPABASE_ANON_KEY`만 넣었다고 해서 채팅·RAG·키가 자동으로 Supabase DB에 들어가지는 않는다. **`DATABASE_URL`이 SQLite면 그 파일이 단일 저장소**이고, Postgres(Supabase)를 쓰려면 `DATABASE_URL`을 그쪽으로 바꾼다. **연결 문자열·SSL·풀러·RLS·마이그레이션 주의**는 [`cap/sql/SUPABASE_SETUP.md`](sql/SUPABASE_SETUP.md)를 따른다.
