...
- **지연 추적(trace)**: 채팅 응답의 단계별 시간(`chat_trace.latency_ms`: topic/RAG/legal/LLM/DB/total)과 감사 로그(`chat.turn.latency`)
...
| `LAW_GO_KR_TIMEBOX_SEC` | 법령 라우팅 전체 시간 상한(초). 초과 시 부분 결과로 조기 종료 (기본 12) |
...
| `MODEL_COMPLEX_PROMOTE` | `true`면 매우 긴 질의에서만 모델을 상위 티어로 조건부 승격(기본 `false`) |
| `RAG_HYBRID_MAX_POOL` | 하이브리드 검색 후보 청크 상한(성능 보호, 기본 1200) |
...
- 현재 백엔드는 OpenAI/Anthropic/Gemini **Hosted API 호출 구조**라, 로컬 추론 서버의 KV cache를 직접 다루지 않습니다.
- 따라서 TurboQuant(로컬 KV cache 양자화)는 **현재 아키텍처에 직접 적용되지 않으며**, 체감 지연 개선은 모델 티어·프롬프트 크기·순차 호출 축소·RAG 경량화가 우선입니다.
- 향후 llama.cpp/vLLM 같은 **로컬 추론 서버를 도입**할 때 TurboQuant를 재평가하는 것이 적합합니다.
- 참고: [TurboQuant 논문](https://arxiv.org/abs/2504.19874), [TurboQuant.cpp](https://github.com/quantumaikr/TurboQuant.cpp)
